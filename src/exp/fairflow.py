...
from models.FairFlow import FairFlow
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Solve paper matching with FairFlow.')
    ...
    debug = config.debug
    assert(config.match_model == 'fairflow' or
           config.match_model == 'fairflow-lb')
    ...
    # Solve.
    mm = FairFlow(loads, loads_lb, covs, scores)
    s = time.time()
    mm.solve()
    t = time.time() - s
